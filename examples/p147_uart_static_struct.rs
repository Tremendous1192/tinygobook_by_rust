...
// UARTドライバオブジェクト
static mut UART_DRIVER: Option<UartDriver> = None;

// UARTドライバオブジェクトの実装
struct UartDriver {
    serial: atsamd_hal::sercom::uart::Uart<
        atsamd_hal::sercom::uart::Config<wio::UartPads>,
        atsamd_hal::sercom::uart::Duplex,
    >,
}
impl UartDriver {
    fn send_message(&mut self, message: &'static [u8]) {
        for c in message.iter() {
            nb::block!(self.serial.write(*c)).unwrap();
        }
    }
}
...
fn main() -> ! {
    ...
    unsafe {
        if let Some(uart_driver) = UART_DRIVER.as_mut() {
            uart_driver.send_message(message.as_bytes());
        }
    }
