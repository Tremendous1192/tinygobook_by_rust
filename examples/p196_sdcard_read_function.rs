...
fn main() -> ! {
    ...
    let sentece_string = read_sentences(&mut sd_controller, &file_name, &mut display);
    draw_text!(sentece_string.as_str(), 4, 2, &mut display);

    // Wi-Fi接続のためのSSID読み込みテスト
    let file_name = "SSID.TXT";
    let sentece_string = read_sentences(&mut sd_controller, &file_name, &mut display);
    draw_text!(sentece_string.as_str(), 4, 2 + 40, &mut display);

    // パスワード
    let file_name = "PASSW.TXT";
    let sentece_string = read_sentences(&mut sd_controller, &file_name, &mut display);
    draw_text!(sentece_string.as_str(), 4, 2 + 60, &mut display);
